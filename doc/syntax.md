...
The `%` post-unary operator is a percentage modifier. If an operand immediately follows `%`, it is instead parsed as the modulo binary operator. For example, `40%` is a fraction, `100 + 40%` is a percentage calculation, and `100 + 6 % 4` uses modulo.
