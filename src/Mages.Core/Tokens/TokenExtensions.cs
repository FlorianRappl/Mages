...
static class TokenExtensions
{
    ...
    public static Boolean IsNeither(this IToken token, TokenType a, TokenType b, TokenType c, TokenType d)
    {
        var type = token.Type;
        return type != a && type != b && type != c && type != d;
    }

    public static Boolean IsNeither(this IToken token, TokenType a, TokenType b, TokenType c)
    {
        var type = token.Type;
