...
These seven kinds of types spawn the whole set of capabilities offered by MAGES. For instance, even if we use a function like `catch` (see [Functions](/docs/functions)) the result will be an `IDictionary<String, Object>` instance. There is no special type returned.
